...
function BlockPreview({block}){const chords=Array.isArray(block.lines)?block.lines.map(line=>line.chords||'').filter(Boolean):[blockChordText(block)].filter(Boolean);return <div className="music-lines">{block.section&&<div className="section-label">[{block.section}]</div>}{chords.map((c,i)=><div className="music-line" key={i}><div className="line-chords">{c}</div></div>)}{blockLyricText(block)&&<div className="line-lyric block-anchor">{blockLyricText(block)}...</div>}</div>}
...
function SelectedSong({song}){let lastSection='';return <section className="print-song"><h2>{song.title}</h2>{song.artist&&<p className="print-artist">{song.artist}</p>}{song.key&&<p className="print-key">Tom: {song.key}</p>}{song.blocks.map((b,i)=>{const showSection=!!b.section&&b.section!==lastSection;lastSection=b.section||lastSection;return <div className="print-block" key={i}>{showSection&&<h3 className="print-section">[{b.section}]</h3>}{Array.isArray(b.lines)&&b.lines.length?b.lines.map((line,j)=><div className="print-line" key={j}>{line.chords&&<div className="print-chords">{line.chords}</div>}{line.lyric&&<div className="print-lyric">{line.lyric}</div>}</div>):<BlockPreview block={b}/>}</div>})}</section>}
